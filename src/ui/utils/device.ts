...
const deviceChecksCache = {
  isMobile: {} as Record<string, IsMobileResult>,
};

function createNavigatorHash(n: Navigator): string {
  return n.userAgent + "-" + n.platform + "-" + n.maxTouchPoints;
}

function createUserAgentMatch(
  userAgent: UserAgent
...
function isMobile(param?: IsMobileParameter): IsMobileResult {
  ...
  const hash = createNavigatorHash(nav);
  const cached = deviceChecksCache.isMobile[hash];
  ...
  const match = createUserAgentMatch(userAgent);
